...
if(isset($_GET['kode'])) {
    ...
    if(isset($_POST['submit'])){
        $kode=mysqli_escape_string($db, $_GET['kode']);
        $nama=mysqli_escape_string($db, $_POST['nama_barang']);
        $kode_gudang=mysqli_escape_string($db, $_POST['kode_gudang']);  

        

        $query=mysqli_query($db, "UPDATE `barang` SET `nama_barang` = '$nama', `kode_gudang`='$kode_gudang' WHERE `barang`.`kode_barang` = '$kode'");
        if ($query) {
    ...
    $sql=mysqli_query($db,"SELECT barang.kode_barang, barang.nama_barang, gudang.lokasi_gudang, gudang.kode_gudang FROM barang INNER JOIN gudang ON barang.kode_gudang=gudang.kode_gudang WHERE barang.kode_barang='$kode'");
            ...
            <tr>
                ...
                <td>
                    <select name="kode_gudang" id="kode_gudang">
                        <?php
                        $query=mysqli_query($db, "SELECT * FROM gudang WHERE kode_gudang='$kode_gudang'");
